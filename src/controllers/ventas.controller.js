...
export const searchVentas = async (req, res) => {
  ...
  const validFields = {
    ...
    producto_id: { column: "producto_id", operator: "=" },
    productos_ids: { column: "producto_id", operator: "IN" },
  ...
  for (const key in searchParams) {
    if (validFields[key]) {
      ...
      console.warn(
        `ADVERTENCIA: Campo de búsqueda no válido '${key}' ha sido ignorado.`
      );
    }
  }
...
export const getTables = async (req, res) => {
  console.log(
    "-> Solicitud GET en /api/tables. Consultando tablas de la base de datos..."
  );
  ...
  try {
    ...
    res.status(500).json({
      status: "error",
      message:
        "Error interno del servidor al obtener las tablas de la base de datos.",
    });
  }
};
...
export const getTableStructure = async (req, res) => {
  ...
  if (!tableName || tableName.trim() === "") {
    return res.status(400).json({
      status: "fail",
      message:
        "El nombre de la tabla es requerido en el cuerpo de la solicitud.",
    });
  }
  ...
  try {
    ...
    console.log(
      `Estructura de tabla '${tableName}' obtenida. Campos encontrados: ${rows.length}`
    );
    ...
    console.error(
      `!!! ERROR EN EL CONTROLADOR [getTableStructure] para tabla '${tableName}':`,
      error.message
    );
    ...
    res.status(500).json({
      status: "error",
      message:
        "Error interno del servidor al obtener la estructura de la tabla.",
    });
  }
};
...
export const searchProductos = async (req, res) => {
  ...
  for (const key in searchParams) {
    if (validFields[key]) {
      ...
      console.log(
        `Procesando campo: ${key} = ${value} (tipo: ${typeof value})`
      );
  ...
  try {
    ...
    console.error(
      `!!! ERROR EN EL CONTROLADOR [searchProductos]:`,
      error.message
    );
    console.error(`Query que falló: ${baseQuery}`);
    console.error(
      `Parámetros que fallaron: [${queryParams
        .map((p) => `${p} (${typeof p})`)
        .join(", ")}]`
    );
...
export const getProductosDisponibles = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: rows,
      message:
        "Productos disponibles para venta (no vendidos, habilitados, no congelados)",
      limited: maxResults ? true : false,
      max_results_applied: maxResults || null,
    });
  } catch (error) {
    console.error(
      "!!! ERROR EN EL CONTROLADOR [getProductosDisponibles]:",
      error.message
    );
    res.status(500).json({
      status: "error",
...
export const getProductosVendidos = async (req, res) => {
  ...
  try {
    ...
    console.error(
      "!!! ERROR EN EL CONTROLADOR [getProductosVendidos]:",
      error.message
    );
    res.status(500).json({
      status: "error",
...
export const getEstadoVentaProducto = async (req, res) => {
  ...
  try {
    ...
    console.error(
      `!!! ERROR EN EL CONTROLADOR [getEstadoVentaProducto]:`,
      error.message
    );
    res.status(500).json({
      status: "error",
...
export const getEstadisticasVentas = async (req, res) => {
  ...
  try {
    ...
    console.error(
      "!!! ERROR EN EL CONTROLADOR [getEstadisticasVentas]:",
      error.message
    );
    res.status(500).json({
      status: "error",
